...
export const loadOrCreateEncryptionKey = () => {
...
export const decrypt = (text, encryptionKey) => {
...
export const executeScriptContent = async (scriptContent, templateName) => {
...
export const runSudoCommand = async (command, args, message) => {
    return new Promise((resolve, reject) => {
        const isTermux = process.env.TERMUX_VERSION !== undefined;
        if (isTermux) {
            console.log(`Termux environment detected. Attempting to run command directly: ${command} ${args.join(' ')}`);
            const child = spawn(command, args, { stdio: 'inherit' });
            child.on('close', (code) => code !== 0 ? reject(new Error(`Command failed with code ${code}: ${command} ${args.join(' ')}`)) : resolve());
        ...
        child.on('close', (code) => code !== 0 ? reject(new Error(`Command failed with code ${code}: sudo ${command} ${args.join(' ')}`)) : resolve());
        child.on('error', (err) => reject(new Error(`Failed to run sudo command: ${err.message}`)));
    });
};
export const applyNixOSConfiguration = async (packageListString, templateName) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (answers.rebuildNixos) {
            ...
            child.on('close', (code) => code !== 0 ? reject(new Error(`NixOS rebuild failed.`)) : resolve());
...
export const registerInstalledPackage = async (packageName, packageHash, aperiumVersion, installedPackagesDir) => {
...
export const createNewApmPackage = async (packageName, encryptionKey) => {
    ...
    try {
        const packageMeta = { name: packageName, version: "1.0.0", description: 'Aperium Package' };
...
export const installFromApmFile = async (apmFilePath, encryptionKey, aperiumVersion, installedPackagesDir) => {
...
export const viewApmFileContent = async (apmFilePath, encryptionKey) => {
    if (!fs.existsSync(apmFilePath) || !apmFilePath.toLowerCase().endsWith('.apm')) {
        console.error(`Error: .apm file not found or invalid extension: "${apmFilePath}".`);
        process.exit(1);
    }
...
export const setupAperiumStructure = (targetFolder) => {
    try {
...
export const executeLegacySetupScript = async (scriptPath, templateName) => {
    if (fs.existsSync(scriptPath)) {
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => code === 0 ? resolve() : reject(new Error(`Legacy script failed with code ${code}`)));
...
export const installTemplateFromDefaultRepo = async (templateName) => {
...
export const ensureSudo = async () => {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => code === 0 ? resolve() : reject(new Error('Sudo access denied.')));
...
export const installPackageFromRepo = async (packageName) => {
    ...
    try {
        const aperiumJson = JSON.parse(fs.readFileSync(aperiumJsonPath, 'utf8'));
...
export const runScript = async (scriptToRun) => {
    const aperiumJsonPath = path.join(process.cwd(), 'aperium.json');
    if (!fs.existsSync(aperiumJsonPath)) {
        console.error('Error: aperium.json file not found. Please run `aper init` first.');
        process.exit(1);
